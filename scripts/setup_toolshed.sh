...
then
    ...
    curl -s -c $cookie -o /dev/null --data "create_user_button=Submit&email=admin%40localhost.com&password=dna2protein&confirm=dna2protein&username=nml&redirect=http%3A%2F%2Flocalhost%3A9009%2F&bear_field=" $api_host/user/create?cntrller=user&use_panels=True 
    sleep 5
    ...
    for i in "${users[@]}" ; do
        curl -s --data "email=$i@localhost.com&password=dna2protein&username=$i" $api_host/api/users?key=$api_key > /dev/null
    done
    ...
    for i in "${cats[@]}" ; do
        curl -s --data "name=$i" $api_host/api/categories?key=$api_key > /dev/null
    done
