...
cd $DEPENDENCIES_INSTALL
python install-workflow-tools.py --workflows-dir workflows/ --master-api-key $MASTER_API_KEY --master-api-url http://localhost:8080/api --galaxy-admin-user admin@localhost.localdomain --galaxy-admin-pass adminpassword | tee install-workflow-tools.log
